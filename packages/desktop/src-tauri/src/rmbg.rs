...
use ndarray::Array;
use ort::{inputs, session::Session, ep};
use std::{fs, path};
...
pub fn process_image(input: &str, model: &str, resolution: u32) -> Result<String> {
    ...
    let output_dir = input_path.parent().ok_or(anyhow!("Download director not found!"))?;
